...
def toplevel_data(title,button_text,command):
    ...
    genderLabel = Label(screen, text='Address', font=('times new roman', 20, 'bold'))
    genderLabel.grid(row=4, column=0, padx=30, pady=15, sticky=W)
    genderEntryEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    genderEntryEntry.grid(row=4, column=1, pady=15, padx=10)
...
def update_data():
    query='update student set name=%s,mobile=%s,email=%s,address=%s,gender=%s,attendance=%s,dob=%s,date=%s,time=%s where id=%s'
...
def search_data():
    query='select * from student where id=%s or name=%s or email=%s or mobile=%s or address=%s or gender=%s or attendance=%s or dob=%s'
    ...
    def connect():
        ...
        try:
            ...
            query='create table student(id int not null primary key, name varchar(30),mobile varchar(10),email varchar(30),' \
                  'address varchar(100)attendance varchar(20),stream varchar(20),gender varchar(20),dob varchar(20),date varchar(50), time varchar(50))'
            mycursor.execute(query)
        except:
            query='use studentmanagementsystem'
            mycursor.execute(query)
